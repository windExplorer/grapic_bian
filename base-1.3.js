...
let config = {
  ...
  /**
   * ** 二级分类计数会有问题, 请把上面的 prefix_category 打开
   *  分类，可酌情修改  
   *  fengjing 风景  
   *  dongman 动漫  
   *  dongwu 动物  
...
let showProgress = () => {
  console.log('[完成信息] 抓取进程：' + flag.successList.length + '/' + flag.planLength)
}
...
let downloadImage = (src, dest, id, callback) => {
  request.head(src, (err, res, body) => {
    ...
    if (src) {
      let req = request(src)
      req.pipe(fs.createWriteStream(dest)).on('close', () => {
          callback(null, id)
      })
      req.on('error', err => {
        console.log(`ERROR: 下载图片 ${id} 出错  正在重试...`)
  ...
  request({
      ...
      if (!err && res.statusCode == 200) {
        ...
        switch(type) {
          ...
          case 2: 
            if(check_had(id)){
              SimpleConsole('[信息](已抓取成功) 退出抓取，文件id为：' + id )
              return
            } else {
      ...
      } else if(res && res.statusCode == 503 && type == 2 && errTime < config.errTimeEnd){
        ...
        if(res && res.statusCode)
          ...
          SimpleConsole(JSON.stringify(res))
