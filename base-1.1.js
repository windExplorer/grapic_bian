 ...
 * 3.简化抓取(原先有3层，1.先获取列表，2.获取列表中图片的真实地址，3.真实地址后面加入固定分辨率再获取图片真实链接) (现2层，1.获取列表，2.获取图片的真实链接)
...
let config = {
  ...
  showConsole: true, //控制台显示抓取信息，[暂时无效，预计后续版本更新]
  user_agent_list: [
  ...
  if(mkdirSync(dir)) {
    fs.exists(file, (exists) => {
      if(!exists) {
        ...
        console.log('[下载信息](文件已存在，若想修复文件请运行 repair.js) 文件id为：' + id)
        flag.successList.push(id)
        showProgress()
  ...
  request({
      ...
      if (!err && res.statusCode == 200) {
        ...
        switch(type) {
          ...
          case 2: 
            if(check_had(id)){
              console.log('[信息](已爬取成功) 退出爬取，文件id为：' + id )
              return
            } else {
              let src = $('.pic img').attr('src')
              if(!src) {
                console.log('[错误](无法获取图片真实链接) id为：' + id + ',链接为：' + src)
                flag.planLength --
              } else {
            ...
            break
          default: 
            console.log('[error](类型错误) 程序退出！')
            break
        }
      } else if(res.statusCode == 503 && type == 2 && errTime < config.errTimeEnd){
        console.log('[重试信息](请求资源失败，状态码 503) 进行第' + errTime+1 +'次重试,id为 ' + id)
